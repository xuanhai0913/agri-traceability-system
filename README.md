...
Repo đã chuẩn bị hướng nâng cấp để demo đạt mức chặt hơn về nghiệp vụ chuỗi cung ứng:
...
| `backend/` | `DATABASE_URL`, RPC URL, `CONTRACT_ADDRESS`, service wallet private key, `PINATA_JWT`, `IPFS_GATEWAY`, `ADMIN_EMAIL`, `ADMIN_PASSWORD`, `JWT_SECRET`, CORS origin. |
...
Backend seed admin từ `ADMIN_EMAIL`, `ADMIN_PASSWORD`, `ADMIN_NAME`. Các tài khoản role dưới đây chỉ được seed tự động khi không chạy production, hoặc khi bật `SEED_DEMO_USERS=true` cho môi trường demo:
